...
const DoneScreen = ({ route }) => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      let ownerTasks = Array.isArray(ownerResponse.data) ? ownerResponse.data.filter(task => task.status === 'DONE') : [];
  
      // Fetch tasks as a group member
      const groupResponse = await axios.get(`http://192.168.1.102:8080/api/tasks/group/user/${id}`);
      let groupTasks = Array.isArray(groupResponse.data) ? groupResponse.data.filter(task => task.status === 'DONE') : [];
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ fontWeight: 'bold' }}>No tasks to show</Text>
          </View>
        }
